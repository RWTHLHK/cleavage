...
registerMooseObject("SolidMechanicsApp", ComputeOrientationCrackedStress);

InputParameters
ComputeOrientationCrackedStress::validParams()
{
  ...
  params.addParam<Real>("beta",1.0,"extend of anisotropy");
  return params;
}

ComputeOrientationCrackedStress::ComputeOrientationCrackedStress(const InputParameters & parameters)
  : DerivativeMaterialInterface<Material>(parameters),
    ...
    _p1_square(declareProperty<Real>("p1_square")),
    _p2_square(declareProperty<Real>("p2_square")),
    _p3_square(declareProperty<Real>("p3_square")),
    _k0(declareProperty<Real>("k0")),
    _cry_rot(getMaterialPropertyByName<RankTwoTensor>(_uncracked_base_name + "crysrot"))
{
}

void
ComputeOrientationCrackedStress::initQpStatefulProperties()
{
  _stress[_qp].zero();
  _hist[_qp] = 0.0;
  _k0[_qp] = 1.0;
...
void
ComputeOrientationCrackedStress::computeQpProperties()
{
  const Real c = _c[_qp];
  ...
  // rotate gradient to crystal coordinates
  RealVectorValue _grad_c_loc = _cry_rot[_qp].transpose()*_grad_c[_qp];
  // compute projection on cleavage planes
  Real norm_square = _grad_c_loc * _grad_c_loc;
  if (norm_square > 0.0){
    _p1_square[_qp] = _grad_c_loc(0) * _grad_c_loc(0) / norm_square;
    _p2_square[_qp] = _grad_c_loc(1) * _grad_c_loc(1) / norm_square;
    _p3_square[_qp] = _grad_c_loc(2) * _grad_c_loc(2) / norm_square;
    _k0[_qp] = 1.0 + _beta * (_p1_square[_qp]*_p2_square[_qp] + 
                        _p1_square[_qp]*_p3_square[_qp] + 
                        _p2_square[_qp]*_p3_square[_qp]);
  }
  
  // Assign L and kappa
  _kappa[_qp] = _k0[_qp] * _gc_prop[_qp] * _l[_qp];
