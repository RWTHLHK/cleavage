...
registerMooseObject("SolidMechanicsApp", ComputeCleavageCrackedStress);

InputParameters
ComputeCleavageCrackedStress::validParams()
{
...
ComputeCleavageCrackedStress::ComputeCleavageCrackedStress(const InputParameters & parameters)
...
void
ComputeCleavageCrackedStress::initQpStatefulProperties()
{
  _stress[_qp].zero();
  _hist[_qp] = 0.0;
}
...
void
ComputeCleavageCrackedStress::computeQpProperties()
{
  const Real c = _c[_qp];
