...
registerMooseObject("SolidMechanicsApp", ComputeCrackedStressCP);

InputParameters
ComputeCrackedStressCP::validParams()
{
...
ComputeCrackedStressCP::ComputeCrackedStressCP(const InputParameters & parameters)
  : DerivativeMaterialInterface<Material>(parameters),
    ...
    _strain(getMaterialPropertyByName<RankTwoTensor>("total_lagrangian_strain")),
...
void
ComputeCrackedStressCP::initQpStatefulProperties()
{
  _stress[_qp].zero();
  _hist[_qp] = 0.0;
}
...
void
ComputeCrackedStressCP::computeQpProperties()
{
  const Real c = _c[_qp];
